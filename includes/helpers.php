...
function renderDescription(string $text): string {
    ...
    while ($i < $n) {
        ...
        // ── STAT LINE: "Strength: 85" or "Strength: 85 — extra text" ────────────
        if (preg_match('/^([A-Za-z][A-Za-z\s&\/\-]+):\s*(\d{1,3})\s*(?:[—\-–]\s*(.+))?$/', $content, $m)) {
            $label = trim($m[1]);
            $val   = (int) $m[2];
            $desc  = trim($m[3] ?? '');

            // If no inline description, look ahead for a plain-text description line
            if ($desc === '') {
                $la = $nextNonEmpty($i + 1);
